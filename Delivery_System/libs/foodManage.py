...
class FoodManage(BaseApi):
    def category_list(self,id):
        ...
        :return: 响应
        """
        return self.request_send(idurl=id)
    def add(self,inData):
        #需要修改参数格式重写数据并调用父类方法
        ...
        return super(FoodManage, self).add(inData)

    def update(self,inData):
        # 需要修改参数格式重写数据并调用父类方法
        inData['specsJson'] = json.dumps(inData['specsJson'])
        return super(FoodManage, self).update(inData)
if __name__ == '__main__':
    #1- 登录成功
    token = Login().login({"username":"ka0181","password":"26205"},getToken=True)
